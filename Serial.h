...
public:
    ...
    typedef enum{
        ...
        EEventNone=0,				// Event trigged without cause事件三角无故
        EEventBreak=EV_BREAK,		// A break was detected on input在输入中检测到一个中断
        EEventCTS=EV_CTS,		// The CTS signal changed state CTS信号改变了状态
        EEventDSR=EV_DSR,		// The DSR signal changed state DSR信号改变了状态
        EEventError=EV_ERR,		// A line-status error occurred 行状态错误发生
        EEventRing=EV_RING,		// A ring indicator was detected 检测到一个环形指示器
        EEventRLSD=EV_RLSD,		// The RLSD signal changed state RLSD信号改变了状态
